# Logbook

| Date  | Used hours | Subject(s) |  Outcome(s) |
| :---         |     :---:      |     :---:      |     :---:      |
| 23.10.2025 | 4 | Preparing the course | Updated content |
| 24.10.2025 | 4 | Preparing the course | Updated tasks  |
| 28.10.2025 | 2 | Kick-off lecture  | Course started |
| 3.11.2025 | 2 | Preparing for the lecture  | Material uptaded |
| 4.11.2025 | 3 | Preparing for the lecture  | Material uptaded |
| 4.11.2025 | 2 | Lecture intro to portswigger  | Lecture regording |
18.2.2026 | 4 | Phase 2 - Booking System | Deployed containers, ran ZAP scan, cracked 6 passwords, completed password security questions |
| 1.3.2026 | 3 | Phase 3 Authorization Testing | Created auth_test_report.md, ran ZAP scan, tested Guest/Reserver/Admin roles |
| 15.3.2026 | 3 | Phase 4 GDPR compliance| nalyzed the booking system for GDPR compliance requirements |
| 22.3.2026 | 5 | PortSwigger Academy Labs | Completed 10 labs: SQL Injection (2), Authentication (2), Access Control (2), Information Disclosure (2), XSS (2) |
| 22.3.2026 | 2 | Phase 3 Resubmit - Authorization Testing | Updated auth_test_report.md, fixed issues, resubmitted assignment |
| 22.3.2026 | 2 | Phase 4 Resubmit - Booking System | Fixed issues, updated security scan documentation, resubmitted assignment |
| 22.3.2026 | 2 | final task report| make the final report  |
